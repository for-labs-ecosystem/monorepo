...
const NETWORK_BRANDS: { name: string; href: string; color: string; desc: string }[] = [
    { name: 'For-Labs', href: 'https://for-labs.com', color: '#2563eb', desc: 'Ekosistem ana merkezi — laboratuvar çözümleri platformu' },
    ...
    { name: 'Alerjen', href: 'https://alerjen.net', color: '#e8612c', desc: 'Hızlı alerjen test kitleri ve gıda güvenliği' },
...
export function NetworkBar() {
    ...
    return (
        <div
            ...
            style={{ background: 'linear-gradient(90deg, #1e3a5f 0%, #1d4ed8 60%, #2563eb 100%)' }}
            ref={menuRef}
        >
            <div className="mx-auto flex h-9 max-w-[1400px] items-center justify-between px-6 lg:px-10">
                ...
                {/* WhatsApp CTA */}
                <a
                    href="https://wa.me/902121234567?text=Merhaba%2C%20bir%20analiz%20cihaz%C4%B1%20hakk%C4%B1nda%20bilgi%20almak%20istiyorum."
                    target="_blank"
            ...
            {/* Dropdown Panel */}
            {isOpen && (
                <div className="absolute left-0 right-0 top-full border-b border-azure-200/30 bg-white/95 backdrop-blur-xl shadow-lg shadow-azure-100/30 z-50">
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
                            {NETWORK_BRANDS.map((brand) => (
                                <a
                                    ...
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    className="group flex flex-col gap-2 rounded-2xl border border-slate-100 bg-white p-4 hover:shadow-lg hover:border-azure-100 transition-all duration-300"
                                >
